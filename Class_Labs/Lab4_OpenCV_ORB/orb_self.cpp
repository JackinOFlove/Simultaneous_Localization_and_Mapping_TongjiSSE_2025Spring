...
const string output_dir = "./output_self";
...
void process_image_pair(const string &img1_path, const string &img2_path, const string &output_path) {
  ...
  // 加载图像
  cv::Mat first_image = cv::imread(img1_path, 0);
  cv::Mat second_image = cv::imread(img2_path, 0);
  ...
  cout << "Image 1: " << keypoints1.size() << " keypoints" << endl;
  cout << "Image 2: " << keypoints2.size() << " keypoints" << endl;
  ...
  // 创建输出目录
  filesystem::path output_dir_path(output_path);
  if (!filesystem::exists(output_dir_path)) {
    filesystem::create_directories(output_dir_path);
  }
  ...
  string match_path = output_path + "/match_" + img1_name + "_" + img2_name + ".png";
  cv::imwrite(match_path, image_show);
  
  cout << "Saved result to " << match_path << endl;
  cout << "----------------------------------------" << endl;
}
